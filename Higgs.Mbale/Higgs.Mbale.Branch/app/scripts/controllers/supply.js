angular
    ...
    function ($scope, $http, $filter, $location, $log, $timeout, $modal, $state, uiGridConstants, $interval,usSpinnerService) {
        ...
        $http.get('/webapi/SupplyApi/GetLatestFiftyNotUsedWeightNoteValuesForAParticularBranch').success(function (data, status) {
...
angular
    .module('homer').controller('UnApprovedSupplyController', ['$scope', 'ngTableParams', '$http', '$filter', '$location', 'Utils', 'uiGridConstants',
        function ($scope, ngTableParams, $http, $filter, $location, Utils, uiGridConstants) {
            $scope.loadingSpinner = true;
            var promise = $http.get('/webapi/SupplyApi/GetAllUnApprovedSuppliesForABranch');
            promise.then(
                function (payload) {
                    $scope.gridData.data = payload.data;
                    $scope.loadingSpinner = false;
